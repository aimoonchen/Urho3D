...
namespace bgfx
{
	...
	const char* s_uniformTypeNameEx[] =
	{
		"int",  "int",
		NULL,   NULL,
		"vec4", "float4",
	...
	BX_STATIC_ASSERT(BX_COUNTOF(s_uniformTypeNameEx) == UniformType::Count*2);
	...
	const char* getUniformTypeNameEx(UniformType::Enum _enum)
	{
		uint32_t idx = _enum & ~(kUniformFragmentBit|kUniformSamplerBit);
		if (idx < UniformType::Count)
		{
			return s_uniformTypeNameEx[idx];
		}

		return "Unknown uniform type?!";
	}

	UniformType::Enum nameToUniformTypeEnumEx(const char* _name)
	{
		for (uint32_t ii = 0; ii < UniformType::Count*2; ++ii)
		{
			if (NULL != s_uniformTypeNameEx[ii]
			&&  0 == bx::strCmp(_name, s_uniformTypeNameEx[ii]) )
			{
...
//		preprocessor.setDefaultDefine("BGFX_SHADER_LANGUAGE_ESSL");
	...
	int32_t memory_writer::write(const void* _data, int32_t _size, bx::Error* _err)
	{
        if (current_size_ + _size > memory_.size()) {
            memory_.resize(int32_t((current_size_ + _size) * 1.5f));
		}
        memcpy(&memory_[current_size_], _data, _size);
        current_size_ += _size;
        return _size;
	}
} // namespace bgfx
