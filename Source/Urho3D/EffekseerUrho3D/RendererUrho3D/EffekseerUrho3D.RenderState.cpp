...
RenderState::RenderState(RendererImplemented* renderer)
    : m_renderer{ renderer }
{
    graphics_ = renderer->GetUrho3DGraphics();
}
...
//                 graphics_->SetBlendModeEx(state1);
            ...
            else
            {
                ...
                // state |= BGFX_STATE_BLEND_EQUATION_SEPARATE(BGFX_STATE_BLEND_EQUATION_ADD,
                ...
                if (m_next.AlphaBlend == ::Effekseer::AlphaBlendType::Blend)
                {
                    ...
                    // BGFX_STATE_BLEND_INV_SRC_ALPHA, BGFX_STATE_BLEND_ONE, BGFX_STATE_BLEND_ONE);
                ...
                else if (m_next.AlphaBlend == ::Effekseer::AlphaBlendType::Add)
                {
                    ...
                    // state |= BGFX_STATE_BLEND_FUNC_SEPARATE(BGFX_STATE_BLEND_SRC_ALPHA, BGFX_STATE_BLEND_ONE,
                    // BGFX_STATE_BLEND_ONE, BGFX_STATE_BLEND_ONE);
                ...
                else if (m_next.AlphaBlend == ::Effekseer::AlphaBlendType::Mul)
                {
                    ...
                    // state |= BGFX_STATE_BLEND_FUNC_SEPARATE(BGFX_STATE_BLEND_ZERO, BGFX_STATE_BLEND_SRC_COLOR,
                    // BGFX_STATE_BLEND_ZERO, BGFX_STATE_BLEND_ONE);
    ...
    if (true/*m_renderer->GetDeviceType() == OpenGLDeviceType::OpenGL3 || m_renderer->GetDeviceType() == OpenGLDeviceType::OpenGLES3*/)
    {
        for (int32_t i = 0; i < (int32_t)m_renderer->GetCurrentTextures().size(); i++)
        {
            ...
            if (m_active.TextureFilterTypes[i] != m_next.TextureFilterTypes[i] || forced ||
                m_active.TextureIDs[i] != m_next.TextureIDs[i])
            {
                ...
                {
                    // GLExt::glSamplerParameteri(m_samplers[i], GL_TEXTURE_MIN_FILTER,
                    // glfilterMin_NoneMipmap[filter_]);
                    if (filter_ == ::Effekseer::TextureFilterType::Nearest)
                    {
