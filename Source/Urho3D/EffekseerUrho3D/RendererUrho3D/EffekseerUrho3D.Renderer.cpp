...
inline Urho3D::Color ConvertColor(const EffekseerRenderer::VertexColor& color)
{
	return Urho3D::Color(color.R / 255.0f, color.G / 255.0f, color.B / 255.0f, color.A / 255.0f);
}

inline Urho3D::Vector2 ConvertUV(const float uv[])
{
	return Urho3D::Vector2(uv[0], uv[1]);
}

inline Urho3D::Vector2 ConvertUV(const Effekseer::Vector2D& uv)
{
	return Urho3D::Vector2(uv.X, uv.Y);
}

inline Urho3D::Vector2 ConvertVertexTextureUV(int32_t offset, int32_t pitch)
{
...
inline Urho3D::Vector3 ConvertVector3(const EffekseerRenderer::VertexFloat3& v)
{
...
//
//----------------------------------------------------------------------------------
RendererRef Renderer::Create(Urho3D::Context* context, int32_t squareMaxCount, int32_t drawMaxCount)
{
    auto renderer = Effekseer::MakeRefPtr<RendererImplemented>(context, squareMaxCount);
...
//
//----------------------------------------------------------------------------------
RendererImplemented::RendererImplemented(Urho3D::Context* context, int32_t squareMaxCount)
    : context_{ context }
...
// 			return false;
	}
	{
		...
		using namespace EffekseerUrho3D::StandardShaders;

		m_shaders[(size_t)RendererShaderType::Unlit] = Shader::Create(graphics_, "Effekseer/Unlit"/*"Sprite_Basic_Unlit"*/, RendererShaderType::Unlit);
		...
		m_shaders[(size_t)RendererShaderType::Lit] = Shader::Create(graphics_, "Effekseer/Unlit"/*"Sprite_Basic_Lighting"*/, RendererShaderType::Lit);
		...
		m_shaders[(size_t)RendererShaderType::BackDistortion] = Shader::Create(graphics_, "Effekseer/Unlit"/*"Sprite_Basic_Distortion"*/, RendererShaderType::BackDistortion);
	...
	const EffekseerRenderer::StandardRendererState& state)
{
	...
	if (shaderType == RendererShaderType::Unlit)
	{
		Urho3D::Vector2* points = pointArray.write().ptr();
		Urho3D::Color* colors = colorArray.write().ptr();
		Urho3D::Vector2* uvs = uvArray.write().ptr();
	...
	const EffekseerRenderer::StandardRendererState& state)
{
	...
	else
	{
		...
		// Copy transfromed vertices
		Urho3D::Vector3* positions = positionArray.write().ptr();
		Urho3D::Vector2* points = pointArray.write().ptr();
		Urho3D::Color* colors = colorArray.write().ptr();
		Urho3D::Vector2* uvs = uvArray.write().ptr();

		const Urho3D::Vector3 frontVec = (flipPolygon) ? Urho3D::Vector3(0.0f, 0.0f, -1.0f) : Urho3D::Vector3(0.0f, 0.0f, 1.0f);
		const Urho3D::Vector3 backVec = (flipPolygon) ? Urho3D::Vector3(0.0f, 0.0f, 1.0f) : Urho3D::Vector3(0.0f, 0.0f, -1.0f);
		...
		int* indices = indexArray.write().ptr();
		Urho3D::Vector3 direction = (state.CullingType == Effekseer::CullingType::Back) ? frontVec : backVec;
...
void RendererImplemented::SetSquareMaxCount(int32_t count)
{
    ...
    auto get_vertex_layout = [](EffekseerRenderer::RendererShaderType shaderType) {
        ...
        case EffekseerRenderer::RendererShaderType::BackDistortion:
            layoutMask = Urho3D::MASK_POSITION | Urho3D::MASK_COLOR | Urho3D::MASK_NORMAL | Urho3D::MASK_TANGENT |
