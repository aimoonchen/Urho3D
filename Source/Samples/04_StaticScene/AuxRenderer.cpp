...
AuxRenderer::AuxRenderer(Context* context)
    : Component(context)
    , lineAntiAlias_(false)
{
    vertexBuffer_ = new VertexBuffer(context_);
    obj_index_buffer_ = new IndexBuffer(context_);
    obj_vertex_buffer_ = new VertexBuffer(context_);
    SubscribeToEvent(E_ENDFRAME, URHO3D_HANDLER(AuxRenderer, HandleEndFrame));
    SubscribeToEvent(E_RENDERPATHEVENT, URHO3D_HANDLER(AuxRenderer, HandleRenderPathEvent));
}

AuxRenderer::~AuxRenderer() = default;

void AuxRenderer::RegisterObject(Context* context)
{
    context->RegisterFactory<AuxRenderer>(SUBSYSTEM_CATEGORY);
    URHO3D_ACCESSOR_ATTRIBUTE("Line Antialias", GetLineAntiAlias, SetLineAntiAlias, bool, false, AM_DEFAULT);
}

void AuxRenderer::SetLineAntiAlias(bool enable)
{
...
void AuxRenderer::SetView(Camera* camera)
{
    if (!camera)
        return;
...
void AuxRenderer::AddLine(const Vector3& start, const Vector3& end, const Color& color, bool depthTest)
{
    AddLine(start, end, color.ToUInt(), depthTest);
}

void AuxRenderer::AddLine(const Vector3& start, const Vector3& end, unsigned color, bool depthTest)
{
...
void AuxRenderer::AddTriangle(const Vector3& v1, const Vector3& v2, const Vector3& v3, const Color& color,
                                bool depthTest)
{
    AddTriangle(v1, v2, v3, color.ToUInt(), depthTest);
}

void AuxRenderer::AddTriangle(const Vector3& v1, const Vector3& v2, const Vector3& v3, unsigned color, bool depthTest)
{
...
void AuxRenderer::AddPolygon(const Vector3& v1, const Vector3& v2, const Vector3& v3, const Vector3& v4,
...
void AuxRenderer::AddNode(Node* node, float scale, bool depthTest)
{
    if (!node)
        return;
...
void AuxRenderer::AddBoundingBox(const BoundingBox& box, const Color& color, bool depthTest, bool solid)
{
...
void AuxRenderer::AddBoundingBox(const BoundingBox& box, const Matrix3x4& transform, const Color& color,
...
void AuxRenderer::AddFrustum(const Frustum& frustum, const Color& color, bool depthTest)
{
...
void AuxRenderer::AddPolyhedron(const Polyhedron& poly, const Color& color, bool depthTest)
{
    unsigned uintColor = color.ToUInt();
...
void AuxRenderer::AddSphere(const Sphere& sphere, const Color& color, bool depthTest)
{
    unsigned uintColor = color.ToUInt();
    ...
    for (auto j = 0; j < 180; j += 45)
    {
        ...
        {
            Vector3 p1 = sphere.GetPoint((float)i, (float)j);
            Vector3 p2 = sphere.GetPoint((float)i + 45, (float)j);
            Vector3 p3 = sphere.GetPoint((float)i, (float)j + 45);
            Vector3 p4 = sphere.GetPoint((float)i + 45, (float)j + 45);
...
void AuxRenderer::AddSphereSector(const Sphere& sphere, const Quaternion& rotation, float angle, bool drawLines,
                                    const Color& color, bool depthTest)
{
    if (angle <= 0.0f)
        return;
...
void AuxRenderer::AddCylinder(const Vector3& position, float radius, float height, const Color& color, bool depthTest)
{
    ...
    for (auto i = 0; i < 360; i += 45)
    {
        Vector3 p1 = sphere.GetPoint((float)i, 90);
        Vector3 p2 = sphere.GetPoint((float)i + 45, 90);
...
void AuxRenderer::AddSkeleton(const Skeleton& skeleton, const Color& color, bool depthTest)
{
...
void AuxRenderer::AddTriangleMesh(const void* vertexData, unsigned vertexSize, const void* indexData,
...
void AuxRenderer::AddTriangleMesh(const void* vertexData, unsigned vertexSize, unsigned vertexStart,
...
void AuxRenderer::AddCircle(const Vector3& center, const Vector3& normal, float radius, const Color& color, int steps,
                              bool depthTest)
{
...
void AuxRenderer::AddCross(const Vector3& center, float size, const Color& color, bool depthTest)
{
    unsigned uintColor = color.ToUInt();
...
void AuxRenderer::AddQuad(const Vector3& center, float width, float height, const Color& color, bool depthTest)
{
    unsigned uintColor = color.ToUInt();
...
void AuxRenderer::Render()
{
    ...
    */
    ...
    int vertex_count = 0;
    for (const auto& objv : obj_vertex_)
    {
        vertex_count += (int)objv.size();
    }
    int index_count = 0;
    for (auto& obji : obj_index_)
    {
        index_count += (int)obji.size();
    }
    if (obj_vertex_buffer_->GetVertexCount() < vertex_count || obj_vertex_buffer_->GetVertexCount() > vertex_count * 2) {
        obj_vertex_buffer_->SetSize(vertex_count, MASK_POSITION | MASK_NORMAL, true);
    }

    if (obj_index_buffer_->GetIndexCount() < index_count || obj_index_buffer_->GetIndexCount() > index_count * 2)
        obj_index_buffer_->SetSize(index_count, true);

//     auto* destv = (AuxObjVertex*)obj_vertex_buffer_->Lock(0, vertex_count, true);
//     if (!destv)
//         return;
// 
//     for (const auto& objv : obj_vertex_) {
//         memcpy(destv, &objv[0], sizeof(AuxObjVertex) * objv.size());
//         destv += objv.size();
//     }
//     obj_vertex_buffer_->Unlock();
//     auto* desti = (unsigned*)obj_index_buffer_->Lock(0, index_count, true);
//     if (!desti)
//         return;
// 
//     for (const auto& obji : obj_index_)
//     {
//         memcpy(desti, &obji[0], sizeof(unsigned) * obji.size());
//         desti += obji.size();
//     }
//     obj_index_buffer_->Unlock();
    ...
    if (!obj_vertex_.empty() || !draw_objects_.empty()) {
		vs = graphics->GetShader(VS, "LitSolid", "NOUV PERPIXEL DIRLIGHT");
		ps = graphics->GetShader(PS, "LitSolid", "AMBIENT PERPIXEL DIRLIGHT");
        ...
        if (light_node_) {
            ...
            // fog
            bool overrideFogColorToBlack = true;// blend == BLEND_ADD || blend == BLEND_ADDALPHA;
            //            graphics->SetShaderParameter(PSP_FOGCOLOR, overrideFogColorToBlack ? Color::BLACK :
            //            zone_->GetFogColor());
            //
            ...
            //             {
            //                 Vector3 worldFogHeightVec = zoneNode->GetWorldTransform() * Vector3(0.0f,
            //                 zone_->GetFogHeight(), 0.0f); fogParams.z_ = worldFogHeightVec.y_; fogParams.w_ =
            //                 zone_->GetFogHeightScale() / Max(zoneNode->GetWorldScale().y_, M_EPSILON);
            //             }
            Vector4 fogParams{2.0f, 1.0f, 0.0f, 0.0f};
            graphics->SetShaderParameter(PSP_FOGPARAMS, fogParams);
            // light
            auto light = light_node_->GetComponent<Light>();
            float atten = 1.0f / Max(light->GetRange(), M_EPSILON);
            Vector3 lightDir(light_node_->GetWorldRotation() * Vector3::BACK);
            //Vector4 lightPos(light_node_->GetWorldPosition(), atten);
            float fade = 1.0f;
        ...
        if (!draw_objects_.empty()) {
			for (auto& obj : draw_objects_) {
				graphics->SetDepthTest(obj.depth_test_ ? CMP_LESSEQUAL : CMP_ALWAYS);
				graphics->SetShaderParameter(VSP_MODEL, obj.world_mat_);
				graphics->SetShaderParameter(PSP_MATDIFFCOLOR, obj.color_);
				graphics->SetVertexBuffer(obj.vertex_buffer_);
				graphics->SetIndexBuffer(obj.index_buffer_);
				graphics->Draw(TRIANGLE_LIST, 0, obj.index_buffer_->GetIndexCount(), 0, obj.vertex_buffer_->GetVertexCount());
			}
        }
        graphics->SetShaderParameter(VSP_MODEL, Matrix3x4::IDENTITY);
        graphics->SetShaderParameter(PSP_MATDIFFCOLOR, Color(1.0f, 1.0f, 1.0f, 1.0f));
        graphics->SetVertexBuffer(obj_vertex_buffer_);
        graphics->SetIndexBuffer(obj_index_buffer_);
        graphics->SetDepthTest(CMP_ALWAYS);
        graphics->Draw(TRIANGLE_LIST, 0, obj_index_buffer_->GetIndexCount(), 0, obj_vertex_buffer_->GetVertexCount());
    }
}

bool AuxRenderer::IsInside(const BoundingBox& box) const { return frustum_.IsInsideFast(box) == INSIDE; }

bool AuxRenderer::HasContent() const
{
    return !(lines_.Empty() && noDepthLines_.Empty() && triangles_.Empty() && noDepthTriangles_.Empty() &&
             obj_vertex_.empty() && draw_objects_.empty());
}

void AuxRenderer::HandleEndFrame(StringHash eventType, VariantMap& eventData)
{
...
//     unsigned obj_vertex_size = obj_vertex_.size();
//     unsigned obj_index_size = obj_index_.size();
    lines_.Clear();
    noDepthLines_.Clear();
    triangles_.Clear();
...
//     if (obj_vertex_.capacity() > obj_vertex_size * 2)
//         obj_vertex_.reserve(obj_vertex_size);
//     if (obj_index_size.capacity() > obj_index_size * 2)
//         obj_index_.reserve(obj_index_size);
}

void AuxRenderer::HandleRenderPathEvent(StringHash eventType, VariantMap& eventData)
{
    if (camera_) {
...
void AuxRenderer::SetLight(Urho3D::Node* lightNode)
{
    if (light_node_ != lightNode) {
        light_node_ = lightNode;
    }
}

void AuxRenderer::SetCamera(Urho3D::Camera* camera)
{
    if (camera_ == camera) {
        return;
    }
...
void AuxRenderer::SetRenderer(Urho3D::Renderer* renderer)
{
    zone_ = renderer->GetDefaultZone();
}

void AuxRenderer::AddDrawObject(Urho3D::Matrix4 worldMat,
    Urho3D::SharedPtr<Urho3D::VertexBuffer> vertexBuffer,
    Urho3D::SharedPtr<Urho3D::IndexBuffer> indexBuffer,
    Urho3D::Color color,
    bool depthTest)
{
    draw_objects_.emplace_back(worldMat, vertexBuffer, indexBuffer, color, depthTest);
}

void AuxRenderer::AddObject(std::vector<AuxObjVertex> object)
{

}

void AuxRenderer::AddObject(std::vector<unsigned int> indices, std::vector<AuxObjVertex> object)
{
    int vertex_count = 0;
    for (const auto& objv : obj_vertex_) {
        vertex_count += objv.size();
    }

    for (auto& idx : indices) {
        idx += vertex_count;
    }
    obj_vertex_.emplace_back(std::move(object));
    obj_index_.emplace_back(std::move(indices));
}
