...
#if defined(SKINNED)
    #define iModelMatrix GetSkinMatrix(a_weight, a_indices)
...
vec3 GetWorldPos(mat4 modelMatrix)
{
        ...
        return GetTrailPos(a_position, a_tangent.xyz, a_tangent.w, modelMatrix);
    #else
        return mul(vec4(a_position, 1.0), modelMatrix).xyz;//(a_position * modelMatrix).xyz;
    #endif
}
...
vec3 GetWorldNormal(mat4 modelMatrix)
{
    ...
    #elif defined(DIRBILLBOARD)
        return GetBillboardNormal(a_position, a_normal, modelMatrix);
    #elif defined(TRAILFACECAM)
        return GetTrailNormal(a_position);
    #elif defined(TRAILBONE)
        return GetTrailNormal(a_position, a_tangent.xyz, a_normal);
    #else
        return normalize(mul(a_normal, GetNormalMatrix(modelMatrix)));
    #endif
}
